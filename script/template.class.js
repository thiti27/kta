...
Template.prototype.createGoogleMaps=function(id)
{
	...
	{
		/* lp */
		var iframe='<iframe style="border:0;width:inherit;height:inherit" src="https://www.google.com/maps/embed/v1/place?key=AIzaSyA2LLXoPbK_YPWDdpwgHaG3-PFYl6KiWhI&center='+$options.googleMap.coordinates_lat+','+$options.googleMap.coordinates_lng+'&q='+$options.googleMap.coordinates_lat+','+$options.googleMap.coordinates_lng+'&zoom=15" allowfullscreen="" frameborder="0"></iframe>';
...
Template.prototype.createMenu=function()
{
	...
	if(parseInt(menu.length,10)!==1) return;
	if(parseInt(menuList.length,10)!==1) return;
...
Template.prototype.preloadPage=function()
{
	...
	{	
		...
		{
			if(parseInt($animationComplete,10)===2)
			{
				var hash=window.location.hash.substring(1);
				if(parseInt($.trim(hash).length)!==0)
...
Template.prototype.filter=function(object)
{
	...
	if(!object.hasClass('filter-0'))
	{
		...
		{
			if(parseInt(aClass[i].indexOf('filter-'),10)!==-1) filter+=' .'+aClass[i];			
		}			
	}
...
{
	object=new String(typeof(object))==='undefined' ? $('.progress-bar') : object;
...
{
	object=new String(typeof(object)==='undefined') ? $('.counter') : object;
...
{	
	...
	if(parentWidth>300)
	{
		...
		if(parseInt(columnCount,10)===0) columnCount=1;
	}
	else columnCount=1;

	if(parseInt(columnCount,10)===1) columnWidth=parentWidth;
...
Template.prototype.createWaypoints=function()
{
	...
	if(parseInt($options.animation.enable,10)!==1)
	{
	...
	{
		if(new String(direction)==='down') $this.animateToCurrentMenuItem($(this));
	},
	{
		offset	:	'50%'
	})
	...
	{
		if(new String(direction)==='up')
		{
			var prevPage=$(this).prev('li');
			if(parseInt(prevPage.length,10)===1) $this.animateToCurrentMenuItem(prevPage);
		}
	},
	{
		offset	:	'50%'
...
{
	if(parseInt($options.parallax.enable)!==1) return;
